...
-- Định nghĩa I/O: Xác định rõ số lượng và loại tham số (IN, OUT, INOUT) cần dùng
-- Đầu vào: p_patient_id,p_phone 
-- Đầu ra : p_total_debt,p_message 

-- Đề xuất 2 giải pháp:
-- Giải pháp 1 : rẽ nhánh 
-- kiểm tra Nếu cả ID và Phone đều NULL thì no sẽ báo lỗi
-- Nếu có id thì truy vấn theo id
-- Nếu không có id nhưng có Phone thì sẽ truy ván cái phone

-- giải pháp 2 : DÙNG WHERE 
-- WHERE (patient_id = p_patient_id OR p_patient_id IS NULL)
--  AND (phone = p_phone OR p_phone IS NULL)

-- SO SÁNH 
-- Độ dễ đọc: giải pháp 1 rox hơn
-- Rủi ro NULL: giải pháp 1 ít hơn 
