...
-- Đây là Stored Procedure đang được sử dụng trên hệ thống. Đoạn code không lỗi cú pháp nhưng có vấn đề về logic:
DELIMITER //
...
-- câu lệnh kiểm tra trạng thái trươcs khi hủy
...
BEGIN
    ...
    IF ROW_COUNT() = 0 THEN
        ...
        SET MESSAGE_TEXT = 'Không thể hủy: Lịch hẹn không ở trạng thái Pending';
    END IF;
END //
DELIMITER ;
