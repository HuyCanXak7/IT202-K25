...
-- Xác định dữ liệu đầu vào và đầu ra. Đề xuất loại tham số phù hợp để vừa nhận dữ liệu, vừa trả về kết quả tính toán?
-- Đầu vào :p_total_cost, p_patient_type
-- Đầu ra :p_final_amount, p_message

-- Đưa ra giải pháp và các bước thực hiện
-- kiểm tra đầu ra p_total_cost
-- nếu nó <= 0 thì phải gán cho cái p_final_amount =0 và cái p_message sễ báo lỗi
-- nếu mọi thứ hợp lệ thì
-- BHYT = p_final_amount = p_total_cost * 0.2
-- VIP = p_final_amount = p_total_cost * 0.9
-- THUONG = p_final_amount = p_total_cost

-- Triển khai mã nguồn theo các bước thực hiện:
DELIMITER //
...
-- Kiểm thử: sử dụng 4 câu lệnh để gọi thủ tục với các dữ liệu để thực hiện kiểm thử cho các trường hợp BHYT, VIP, THUONG và chặn + thông báo lỗi đúng khi dữ liệu tổng chi phí là âm

CALL CalculateFinalCost(1000000, 'BHYT', @final_amount, @msg);
SELECT @final_amount, @msg;

CALL CalculateFinalCost(500000, 'VIP', @final_amount, @msg);
SELECT @final_amount, @msg;

CALL CalculateFinalCost(300000, 'THUONG', @final_amount, @msg);
SELECT @final_amount, @msg;

CALL CalculateFinalCost(-500000, 'BHYT', @final_amount, @msg);
SELECT @final_amount, @msg;
