...
-- Đầu vào :p_patient_id,p_medicine_id,p_quantity,p_discount_code
-- Đầu ra :p_message 

-- Khởi tạo nhanh 2 bảng mẫu (Có chèn sẵn vài dòng dữ liệu để test)
CREATE TABLE Medicines (
    medicine_id INT PRIMARY KEY,
    name VARCHAR(100),
    price DECIMAL(18,2),
...
-- Viết mã SQL khởi tạo Procedure ProcessPrescription bám sát luồng thiết kế
DELIMITER //
...
-- Viết 3 câu lệnh CALL để chứng minh hệ thống xử lý thành công 3 kịch bản: (1) Kê đơn bình thường không có mã giảm giá; (2) Kê đơn có mã NV-RIKKEI; (3) Kê đơn vượt quá số lượng tồn kho (Bẫy 1)
