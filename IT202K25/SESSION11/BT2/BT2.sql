...
-- Đoạn code đang chạy trên hệ thống không có lỗi cú pháp, nhưng đang gây ra sai lệch dữ liệu trong một số trường hợp thực tế:
DELIMITER //
...
-- KHI NHẬP SỐ ÂM HỆ THỐNG SẼ TRỪ VAO KHO LÀM CHO BỊ MẤT HÀNG CHỨ KHÔNG BÁO LỖI

-- Viết lệnh DROP để xóa thủ tục cũ.
DROP PROCEDURE IF EXISTS AddInventory;

-- Viết lệnh CREATE tạo lại thủ tục AddInventory mới. Cấu trúc mã phải đảm bảo: Chỉ thực hiện cộng kho nếu dữ liệu truyền vào hợp lệ theo đúng quy tắc hệ thống.
DELIMITER //
...
BEGIN
    ...
    IF p_quantity <= 0 THEN
        ...
        SET MESSAGE_TEXT = 'Lỗi: Số lượng nhập kho phải lớn hơn 0';
    ELSE
        UPDATE Inventory
        SET stock_quantity = stock_quantity + p_quantity
        WHERE item_id = p_item_id;
